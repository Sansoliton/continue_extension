...
const GenerateTestMethodologySlashCommand: SlashCommand = {
  name: "generateTestMethodology",
  description: "Generates Test Methodology",
  run: async function* (sdk) {
    ...
    const BACKEND_SERVER_URL = "http://vina-copilot-server.azurewebsites.net/ai/measurement/retrieve_procedure";
    try {
      ...
      const filePath = `${currentDirectory}/src/measurement_methodology/test_methodology.txt`;
      const fileContent = serverResponse;

      await ide.writeFile(filePath, fileContent);
      await ide.openFile(
        `${currentDirectory}/src/measurement_methodology/test_methodology.txt`,
      );
...
export default GenerateTestMethodologySlashCommand;
